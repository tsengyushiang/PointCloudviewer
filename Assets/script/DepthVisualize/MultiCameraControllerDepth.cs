...
public class MultiCameraControllerDepth : MonoBehaviour {
    ...
    public bool isDepthMode = true;

    public Camera mainCamera;

    private Camera[] cameras;
    private DepthVisualization[] depthScript;
    ...
    void Awake() {
        ...
        depthScript = new DepthVisualization[cameras.Length];
        ...
        for (int i = 0; i < normalCameraCount; ++i) {
            ...
            renderTextures[i] = new RenderTexture(resWidth, resHeight, 0, RenderTextureFormat.Default);
            renderTextures[i].Create();
            depthScript[i] = cameras[i].GetComponent<DepthVisualization>();
    ...
    void Update() {
        for (int i = 0; i < cameras.Length; ++i) {
            depthScript[i].enabled = isDepthMode;
            //manual render
    ...
    void SaveJson(int len) {
        ...
        for (int i = 0; i < cameras.Length; ++i) {
            ...
            for (int frame = 0; frame < len; frame++) {
                if(isDepthMode)
                    cam.img[frame] = "/d" + i.ToString() + "_" + frame + ".png";
                else
    ...
    void cacheScreenShot() {
        Debug.Log("save");
        for (int i = 0; i < cameras.Length; ++i) {
            camData[i].Add(saveRenderTextureToByte(renderTextures[i]));
        }
    }
    ...
    void SaveScreenShot() {
        for (int camIndex = 0; camIndex < camData.Count; camIndex++) {
            for (int dataIndex = 0; dataIndex < camData[camIndex].Count; dataIndex++) {
                string imgPath = "null";
                if (isDepthMode)
                    imgPath = saveByteToFile(camData[camIndex][dataIndex], "/d" + camIndex.ToString() + "_" + dataIndex + ".png");
                else
                    imgPath = saveByteToFile(camData[camIndex][dataIndex], "/" + camIndex.ToString() + "_" + dataIndex + ".png");
                Debug.Log(imgPath);
            }
        }
    }
